...
extractFeatures1 <- function(DT_in, features = c(  
  'Gender'
  , "AgeYears", "Institution","Group", "Region","DaysSince", "avgScore", "corrected_mean_duration_person",
  c(paste0('corrected_Du0', 1:9), paste0('corrected_Du', 10:32))
...
#using extractFeatures2 for presentation
...
p1 <- ggplot(featureImportance, aes(x=reorder(Feature, Importance), y=Importance)) +
  geom_bar(stat="identity", fill="#53cfff") +
  coord_flip() + 
  theme_light(base_size=20) +
  xlab("") +
