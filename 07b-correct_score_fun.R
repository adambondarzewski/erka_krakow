...
calculate_score <- function(DT, w, v, lambda, score_fun = score) {
    
    DT$score_measured <- apply(DT_shrinked, 1, function(x) {score_fun(x, w = w, v = v, lambda)})
    ...
    DT_groups_casted[, gender_gap := (`F` - `M`)^2 * participants_sum]
